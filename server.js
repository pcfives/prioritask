...
app.post('/analyze', async (req, res) => {
  // 1) Guard against missing or non-array tasks
  const tasks = Array.isArray(req.body.tasks) ? req.body.tasks : [];

  // 2) Build list of uncompleted tasks
  ...
  // 3) If none, return empty
  if (relevantTasks.length === 0) {
    return res.json({ assignments: [] });
  }

  // 4) Build strengthened prompt
...
.trim();
  ...
  try {
    ...
    // 7) Pull out the raw assistant text
    let content = data.choices?.[0]?.message?.content?.trim() || '';

    // 7a) Strip fenced code blocks if present
    const fenceRe = /```(?:json)?\s*([\s\S]*?)\s*```/i;
    const fenceMatch = content.match(fenceRe);
    if (fenceMatch) content = fenceMatch[1].trim();

    // 7b) Bracket-matching JSON extraction
    ...
    // 9) Normalize singular → plural if needed
...
app.post("/generate-schedule", async (req, res) => {
  ...
  // Filter and format only uncompleted tasks
  const relevantTasks = tasks
  .filter(t => !t.completed)
  .map((t, i) => {
